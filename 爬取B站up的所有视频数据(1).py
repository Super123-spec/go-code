...
def getallvideos():                     # 爬虫的主函数
    mid = input("请输入B站up主的mid:（默认:1749217079）")                          # 输入up的mid
    if mid=="":
        mid='661047235'
    ...
    try:                                        # 如果用户输入up的mid不正确，这里就无法获取数据，
        ...
        print('序号', 'up主昵称', '视频标题', '视频的评论数', '视频封面图片的地址', 'up的mid', '视频发布时间', '视频时长', '视频的弹幕数', '视频地址')
        i = 1                                  # 计数变量
        for currentpage in range(1, page + 1):      # 遍历每一页
            ...
            for li in list:                             # 遍历每一个视频
                ...
                print(i, author, title, comment, post, mid, created, length, video_review, src)
                i = i + 1                               # 计数 +1
        time.sleep(0.3)     # 由于爬虫的速度太快了，这里等待一下下，防止被服务器抓到
    except:                 # 程序出错了
        print("呜呜呜，程序出错了，请检查up的mid是否输入正确，或者是我们的爬虫遭到了拦截，更多帮助，请添加微信：safeseaa  ")
